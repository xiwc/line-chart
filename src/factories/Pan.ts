...
export class Pan extends BaseFactory {
  ...
  move(domains: Utils.IDomains) {
    ...
    var xAxis = <Axis>this.factoryMgr.get('x-axis');
    var x2Axis = <Axis>this.factoryMgr.get('x2-axis');
    var yAxis = <Axis>this.factoryMgr.get('y-axis');
    var y2Axis = <Axis>this.factoryMgr.get('y2-axis');
  ...
  getNewDomains(deltaX: number, deltaX2: number, deltaY: number, deltaY2: number): Utils.IDomains {
    let xAxis = <Axis>this.factoryMgr.get('x-axis');
    let x2Axis = <Axis>this.factoryMgr.get('x2-axis');
    let yAxis = <Axis>this.factoryMgr.get('y-axis');
    let y2Axis = <Axis>this.factoryMgr.get('y2-axis');
